...
namespace PokerHandsComparer
{
    class Hand
    {
        ...
        public CategoryRank GetCategoryRank()
        {
            ...
            if (IsRoyalFlush())
                return CategoryRank.RoyalFlush;
            ...
            return IsPair() ? CategoryRank.Pair : CategoryRank.HighCard;
        }

        private bool IsRoyalFlush()
        {
            return IsStraight() && IsFlush() && Cards.All(card => card.Rank >= Rank.Ten);
        }

        private bool IsStraightFlush()
        {
            return IsStraight() && IsFlush() && Cards.Any(card => card.Rank < Rank.Ten);
        }
        ...
        private bool IsStraight()
        {
            ...
            return !Cards.Select((card, index) => card.Rank - index).Distinct().Skip(1).Any() || isStraightFromAce;
        }
        ...
        private bool IsPair()
        {
            var towOfAKindGroups = Cards.GroupBy(card => card.Rank).Where(group => group.Skip(1).Any()).ToList();
